...
req1<-summary(responses[,.(frame.number, frame.time_relative, frame.time_delta, frame.len, ip.proto, ip.version)])
kable(req1, caption="Requests Summary")

req2<-summary(responses[,.(ip.src, eth.src, ip.dst, eth.dst)])
kable(req2)
                      
req3<-summary(responses[,.(mbtcp.modbus.unit_id, tcp.srcport, tcp.dstport, mbtcp.prot_id, mbtcp.trans_id, mbtcp.len)])
kable(req3)

req4<-summary(responses[,.(mbtcp.modbus.func_code, mbtcp.modbus.reference_num, mbtcp.modbus.word_cnt)])
kable(req4)

#req5<-summary(responses[,.(mbtcp.modbus.data, frame.second)])
#kable(req5)

#print(xtable(summary(reqs)), floating.environment="sidewaystable", comment=FALSE)
...
tab<-summary(reqs[,.(frame.number, frame.time_relative, frame.time_delta)])
#print(xtable(tab), floating.environment="sidewaystable", comment=FALSE, size="small")
#sideways <- xtable(summary(reqs))
#sideways
#print(sideways, floating=FALSE)
