...
```{r, echo=FALSE, warning=FALSE, fig.pos='bottom', fig.width=4, fig.height=5, fig.align='center', fig.cap="OSI Model"}
...
```{r, echo=FALSE, fig.pos='bottom', fig.width=4.5, fig.height=3.45, fig.align='center', fig.cap="IPv4 Header"}
...
```{r, echo=FALSE, fig.pos='bottom', fig.width=5.5, fig.width=4.5, fig.align='center', fig.cap="Modbus TCP/IP Communication Architecture"}
...
It is important to note that in identifying Modbus conversations, the listening port over TCP/IP reserved for Modbus communication is 502. Its services are specified by indicating the function code in the Modbus request and response. Appendix A lists the public function codes available.

As a result of using the simple design of Modbus and its reliance on the TCP layer, security issues become inherent. With no authentication and authorization mechanisms, data integrity checks, or encryption, the SCADA network components provide no identity or permission verification, message content legitimacy, or confidentiality.
...
In a reconnaissance attack there is unauthorized reading of data, where this type of attack generally surveys a network and identifies connected devices in order to ascertain the network architecture and topology. Once the attacker gains access to the network, they may carry out different levels of scanning over the network, such as address, port and endpoints scanning. In the case of SCADA systems, with the Modbus/TCP being the prevailing protocol implemented in ICS devices, function scanning is also attempted.
...
```{r, echo=FALSE, message=FALSE, warning, fig.height=5, fig.width=5, fig.align='center', fig.cap="SCADA Network Graph"}
...
kable(sewModbusDT[,.(count=.N), by=.(ip.src, ip.dst, mbtcp.modbus.unit_id)], caption='Source / Destination / UnitID')
```
  
```{r, comment='', echo=FALSE, warning=FALSE, message=FALSE}
kable(sewModbusDT[,.(count=.N), by=.(ip.src)], caption='Sources')
```

```{r, comment='', echo=FALSE, warning=FALSE, message=FALSE}
kable(sewModbusDT[,.(count=.N), by=.(ip.dst)], caption='Destination')
```

\newpage

```{r, comment='', echo=FALSE, warning=FALSE, message=FALSE}
kable(sewModbusDT[,.(ip.dst.unit_id = paste(ip.dst, mbtcp.modbus.unit_id, sep="/")),
                    by=.(ip.dst, mbtcp.modbus.unit_id)], caption='Destination / UnitID')
```
  
```{r, comment='', echo=FALSE, warning=FALSE, message=FALSE}
kable(sewModbusDT[,.(src.func = paste(ip.src, mbtcp.modbus.func_code, sep="/"))
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.width=4.5, fig.height=4, fig.align='center', fig.cap="Source Packet Size Boxplot"}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.pos='top', fig.width=5, fig.height=3.45, fig.align='center', fig.cap="Boxplot of Request Packets"}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.pos='bottom', fig.width=5, fig.height=3.45, fig.align='center', fig.cap="Boxplot of Response Packets"}
...
```{r, echo=FALSE, fig.pos='middle', message=FALSE, warning=FALSE, fig.align='center', fig.cap="3D Scatterplot of Modbus Reference Number\n and Data Over Time for Function Code 4"}
...
It can be seen in Figures 18 and 22 how cyclical the processes are in the system.
...
```{r, echo=FALSE, fig.pos='bottom', fig.width=5.5, fig.height=2.70, fig.align='center', fig.cap="Technical Architecture"}
...
```{r, echo=FALSE, fig.pos='bottom', fig.width=5.75, fig.height=4.25, fig.align='center', fig.pos='bottom', fig.cap="Process Flow"}
#setwd("~/rDia")
...
As discussed in Section 9.2.1, the process flow depicts the phases of the processes involved, from data acquisition, to data processing and statistical analysis, and finally to placing the device in “operational” mode where network traffic may be exposed to attacks. During the testing phase, another simulation was done with attacks injected via Metasploit. The packet capture file was also processed in a similar manner as the initial pcap file, and then a comparison was made to find differences and anomalies between the two data sets.

The test simulation exposed the routine activity running on the Virtual Scada Box to a Modbus STOP attack. The commands executed from Metasploit were a series of payloads sent by the attacker in order to establish the initial state of the water tank. This was then followed by the stop command with its relevant data values. (Appendix F)
...
In Table 7, we see Modbus function code 90 sent to perform administrative commands without authentication.
...
The difference in average frequency shown in Table 9 are indicated by a prefix 'n' for normal and 'a' for attack.
...
## Wireshark[^4] - Network Traffic Analysis Tool
[^4]: https://www.wireshark.org/docs/wsug_html_chunked
...
Another tool from the Wireshark suite is the command-line tool similar to tcpdump[^6] is tshark, a network protocol analyzer. In addition to capturing packet data over a live network, it is also capable of analyzing packets from an existing capture file.
...
RStudio[^8]  is an integrated development environment for the R statistical programming language.
...
## Suricata[^9]
[^9]: http://www.suricata-ids.org

A widely used network intrusion detection system is Snort.[^10] It is open source, and although highly effective and proven, its limitation is that it is single-threaded. A newer generation NIDS that has grown fairly popular is Suricata, developed by the Open Information Security Foundation (OISF) and partly funded by the US Department Homeland Security’s Directorate for Science and Technology. 
...
[^11]: http://www.metasploit.com/index.jsp
